...
function obtainScript(content, resource, opts) {
  return content.replace(rScript, function(all, comment, script, body) {
    ...
    if (opts.obtainScript && rRosettaType.test(script)) {
      resource.addJsEmbed(body);
      all = '';
    } else if (!body.trim() && rSrcHref.test(script)) {
      var src = RegExp.$2;
...
function obtainStyle(content, resource) {
  return content.replace(rLink, function(all, comment, link) {
    if (!comment && rRefStyle.test(link) && rSrcHref.test(link)) {
      var href = RegExp.$2;
...
function beforeProcess(file, resource, opts) {
  ...
  if (opts.obtainStyle) {
    content = obtainStyle(content, resource);
  }
