...
Resource.prototype.add = function(id, deffer) {
  ...
  if (pkg) {
    ...
    if (node.has && node.has.length) {
      ...
      if (deffer && node.type === 'js') {
        self.asyncs.push(res);
      }
    }
  } else {
    loaded[id] = deffer;
  ...
  switch (type) {
    case 'js':
      this[deffer ? 'asyncs' : 'js'].push(deffer ? id : uri);
      break;
    case 'css':
      this.css.push(uri);
      break;
...
Resource.prototype.removeDefferFromList = function(id) {
  ...
  var idx = this.asyncs.indexOf(id);
  if (~idx) {
  ...
  if (node.pkg) {
    ...
    node.has && node.has.forEach(function(res) {
      var idx = self.asyncs.indexOf(res);
      if (~idx) {
...
Resource.prototype.buildResourceMap = function() {
  ...
  this.asyncs.forEach(function(id) {
